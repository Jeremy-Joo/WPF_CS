...
if (args.Length > 0 && args[0] == "population")
{
    ...
    // 결과폴더 아래 .cache 에 **영구** 저장/재사용 — 한 번 받은 사람은 다시 안 받는다.
    // (매일 돌려도 2천 명을 매번 다시 긁던 문제의 해법. 특정 사람을 최신화하고
    // 싶으면 그 사람의 .cache\<식별코드>.json 을 지우고 다시 돌릴 것.)
    ...
    for (int i = 0; i < targets.Count; i++)
    {
        ...
        try
        {
            var replays = await WavuApiCollector.FetchWithCacheAsync(
                it.Id, popCache, null, Console.WriteLine);
